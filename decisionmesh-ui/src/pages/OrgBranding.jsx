...
import { Upload, Palette, Type, Check, RefreshCw, Building2, Eye } from 'lucide-react';
...
async function uploadLogo(keycloak, file) {
  ...
  if (res.ok) {
    ...
    return data?.logoUrl ?? URL.createObjectURL(file);
  }
  // API not ready — use object URL for preview
  return URL.createObjectURL(file);
}

async function saveBranding(keycloak, body) {
  try {
    await fetch('http://localhost:8080/api/org/branding', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${keycloak?.token}` },
      body: JSON.stringify(body),
    });
  } catch { /* persist locally via context regardless */ }
}
...
export default function OrgBranding({ keycloak }) {
  ...
  async function handleLogoChange(e) {
    ...
    setUploading(true);
    const url = await uploadLogo(keycloak, file);
    setPreview(url);
  ...
  function handleColorSelect(color) {
    ...
    // Live preview — apply immediately but don't save yet
    updateBranding({ primaryColor: color });
  }

  async function handleSave() {
    setSaving(true);
    await saveBranding(keycloak, form);
    updateBranding(form);
    setSaving(false);
    setSaved(true);
            ...
            <CardContent>
              <div className="flex items-center gap-5">
                ...
                <div className="space-y-2">
                  ...
                  {preview && (
                    <button onClick={() => { setPreview(null); setForm(f => ({ ...f, logoUrl: null })); }}
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    placeholder="#2563eb"
                    maxLength={7}
                    className="w-28 text-sm font-mono border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    style={{ fontFamily: "'JetBrains Mono', monospace" }}
                  />
                  <div className="flex-1 h-8 rounded-lg border border-slate-100" style={{ background: form.primaryColor }} />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="flex h-64" style={{ background: '#f8fafc' }}>
              <div className="w-40 bg-white border-r border-slate-100 flex flex-col">
                ...
                <div className="flex items-center gap-2 px-3 py-3 border-b border-slate-100">
                  ...
                  <span className="text-xs font-semibold text-slate-800 truncate">{form.orgName || 'Your Org'}</span>
                </div>
                {/* Nav items */}
              ...
              <div className="flex-1 p-3 space-y-2">
                ...
                <div className="flex items-center justify-between bg-white rounded-lg px-3 py-1.5 border border-slate-100">
                  ...
                  <div className="w-5 h-5 rounded-full shrink-0" style={{ backgroundColor: form.primaryColor, opacity: 0.8 }} />
                </div>
                {/* KPI cards */}
                <div className="grid grid-cols-2 gap-1.5">
                  {['Intents', 'Cost', 'Success', 'Drift'].map(label => (
                    <div key={label} className="bg-white rounded-lg p-2 border border-slate-100">
                      <div className="w-4 h-1 rounded-full mb-1.5" style={{ backgroundColor: form.primaryColor, opacity: 0.3 }} />
